...
class DiagnosisGenerateTextView(APIView):
    ...
    Генерирует шаблон текста для поля «Комментарий» на основе данных карты.
    Врач может принять его или написать своё. Итоговый диагноз по МКБ-10
    вставляется в PDF автоматически — отдельно от этого поля.
